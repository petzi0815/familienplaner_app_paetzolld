...
/// bald ablaufende Lebensmittel (Vortag 09:00) + Abfuhr (Vortag 19:00).
/// Termine laufen über den serverseitigen Per-User-Push (2 & 1 Tag vorher, opt-in) → hier bewusst NICHT
/// mehr lokal, um Doppel-Benachrichtigungen zu vermeiden.
/// Idempotent: eigene Requests (Präfix) werden vor dem Neuplanen entfernt.
enum LocalReminders {
    private static let prefix = "fp-local-"

    static func reschedule(vorrat: [VorratShort], abfuhr: [AbfuhrNext] = []) {
