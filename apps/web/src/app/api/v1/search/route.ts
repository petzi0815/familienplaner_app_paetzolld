...
// Cross-Domain-Volltextsuche (LIKE über Text-Spalten). Für den Agenten + globale UI-Suche.
export const runtime = "nodejs";
export const dynamic = "force-dynamic";

const DISPLAY = ["title", "titel", "name", "friendly_name", "bezeichnung", "anbieter", "beschreibung", "problem"];
function display(row: Record<string, unknown>): string {
  for (const c of DISPLAY) if (row[c]) return String(row[c]);
  return "";
}
...
export function GET(req: Request): Response {
  ...
  const perResource = Math.min(Math.max(Number(url.searchParams.get("limit") ?? 8) || 8, 1), 50);
  ...
  for (const r of RESOURCES) {
    ...
    try {
      const rows = db.prepare(`SELECT * FROM "${r.table}" WHERE ${whereSql} LIMIT ?`).all(...params, perResource) as Record<string, unknown>[];
      for (const row of rows) results.push({ resource: r.key, domain: r.domain, label: r.label, id: row[pkOf(r)], display: display(row) });
    } catch { /* Tabelle überspringen */ }
