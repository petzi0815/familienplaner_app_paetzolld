...
- **OFFEN — Kameras (Nutzer will es, nur recherchiert):** Anbindung über **Home Assistant** (Snapshot `/api/camera_proxy/` + **Live-HLS** via WS `camera/stream`, beides über DuckDNS verifiziert — **KEIN Tailscale, kein neuer Key**). Der gelieferte UniFi-Cloud-Key = 403 insufficient (Cloud) / 401 (lokaler Controller `192.168.57.1`); Cloud kann kein Live (RTSPS-URLs lokal). Prio: **Schnappschuss-Raster + Live-Video (Tap)** ins Smarthome-Tab. Bauplan in [[reference-unifi-protect-cameras]].
- **OFFEN (Lars, Coolify):** `HOME_ASSISTANT_URL` + `HOME_ASSISTANT_TOKEN` sind gesetzt ✓. Optional `ALARMO_PIN` in Coolify setzen, wenn der PIN NICHT im Repo stehen soll (sonst greift Default 4578).
