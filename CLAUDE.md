...
- **Kameras GEBAUT (über Home Assistant, kein UniFi-Key/Tailscale):** `server/homeassistant/cameras.ts` (Allow-List 9 Kameras) + Routen `GET /api/v1/smarthome/cameras`, `/cameras/[entity]/snapshot` (Backend proxyt JPEG mit HA-Token), `/cameras/[entity]/stream` (Backend mintet **Live-HLS** via HA-**WebSocket** `camera/stream` → absolute URL, AVPlayer spielt nativ). iOS: Kamera-Sektion im Smarthome-Tab — `CameraThumb` (Snapshot-Raster, alle 6 s, scenePhase-gekoppelt) + `CameraLiveView` (AVKit `VideoPlayer`, Vollbild). Alle Endpunkte gegen echtes HA verifiziert (9 Kameras, Snapshot 200, HLS-Playlist 200, unbekannt→422). Recherche/Warum-HA in [[reference-unifi-protect-cameras]].
...
- **OFFEN (Lars, Coolify):** `HOME_ASSISTANT_URL` + `HOME_ASSISTANT_TOKEN` gesetzt ✓. Optional `ALARMO_PIN` in Coolify, wenn der PIN NICHT im Repo stehen soll (sonst Default 4578). Coolify muss HA (`petzi0815.duckdns.org:8123`) per https+wss erreichen (für Alarmo/Raffstore/Kamera-Snapshot/HLS-Mint).
